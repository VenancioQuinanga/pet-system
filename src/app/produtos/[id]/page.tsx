...
export default function EditProduct({params}:{params:{id:any}}) {
  ...
  const handleSubmit = (e:FormEvent<HTMLFormElement>)=>{
    ...
    editProduct(product, params.id, token)
  }
  ...
  useEffect(()=>{
    const token = localStorage.getItem('token')
    getProduct(setProduct, params.id, token) 
    getProductsTypes(setTypes, token) 
    getProductsFamilies(setFamilies, token) 
    getProductsProvisioners(setProvisioners, token) 
  },[])
