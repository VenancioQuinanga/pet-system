...
export default function useProformActions() {
  ...
  async function getSales(setSales: Function, token: any): Promise<void> {
    try {
      const res = await api.get('/venda_proforma', {
        headers: {
  ...
  async function getSale(setSale: Function, id: number, token: any): Promise<void> {
    try {
      const res = await api.get(`/venda_proforma/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      try{
        api.post(`/venda_proforma`, data, {
          ...
          .then((res)=>{
            ...
            let date = new Date()
            let code = `FP/${date.getFullYear()}/${res.data.sale.id}`
            let invoice = {
              fk_proform: res.data.sale.id, 
              code: code,
              client_name: sale?.client_name, 
              client_nif: sale?.client_nif
            }
            addInvoice(invoice, token)
            router.push(`/faturacao/proforma/${res.data.sale.id}`)
