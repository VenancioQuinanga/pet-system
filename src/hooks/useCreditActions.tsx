...
export default function useCreditActions() {
  ...
  async function getSales(setSales: Function, token: any): Promise<void> {
    try {
      const res = await api.get('/venda_credito', {
        headers: {
  ...
  async function getSale(setSale: Function, id: number, token: any): Promise<void> {
    try {
      const res = await api.get(`/venda_credito/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      try{
        api.post(`/venda_credito`, data, {
          ...
          .then((res)=>{
            ...
            let date = new Date()
            let code = `FC/${date.getFullYear()}/${res.data.sale.id}`
            let invoice = {
              fk_credit: res.data.sale.id, 
              code: code,
              client_name: sale?.client_name, 
              client_nif: sale?.client_nif
            }
            addInvoice(invoice, token)
            router.push(`/faturacao/credito/${res.data.sale.id}`)
